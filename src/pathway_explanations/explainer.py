...
class Explainer:
    ...
    def extract_index(element, names=None):
        ...
        """
        # TODO: expand to more than one gene, in a future iteration
        if names is None:
            # If no names are given, assume the gene name is already
            # a gene index
            assert isinstance(element, int) or isinstance(
                element, float
            ), "No element names have been given and the gene name given is not numeric"
            return int(element)
